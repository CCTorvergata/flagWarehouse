...
def run_exploit(exploit_path,
    ...
    for round_num in rounds:
        ...
        p = None
        try:
            p = subprocess.Popen([exploit_path, team_ip, str(
                round_num)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            timer = Timer(math.ceil(0.9 * round_duration /
                          len(rounds)), timer_out, args=[p])
            timer.start()
            ...
            for line in iter(p.stdout.readline, b''):
                ...
                for f in set(flag_pattern.findall(output)):
                    ...
                    current_max_round, _ = last_success_memory.get(
                        memory_key, (-1, None))
                    ...
                    if current_round_int > current_max_round:
                        last_success_memory[memory_key] = (
        ...
        try:
            ...
            while not flag_queue.empty():
                try:
                    ...
                    # La coda è stata svuotata da un altro processo.
                    # Nel frattempo, usciamo dal ciclo
                    break
...
def main(args):
    ...
    try:
        ...
        logging.error(
            f"Impossibile ottenere la configurazione dal server: {e}")
        sys.exit(1)
    ...
    try:
        ...
        submitter_thread = Thread(target=flag_submitter,
                                  args=(flag_queue,
                                        server_url,
                                        token,
                                        user,
        ...
        while True:
            ...
            if not scripts:
                logging.warning(
                    "Nessun exploit .py trovato. Riprovo tra 15 secondi.")
                time.sleep(15)
                continue

            try:
                flag_ids_resp = requests.get(
                    'http://10.10.0.1:8081/flagIds?team=0', timeout=5)
            ...
            # Create a non-proxy copy for iteration
            current_exploit_map = dict(exploit_to_service_map)
            ...
            unknown_exploits = [
                s for s in scripts if s not in current_exploit_map]
            if unknown_exploits:
            ...
            for team in teams:
                ...
                for service_id in all_known_services:
                    ...
                    if service_id != 'unknown':
                        ...
                        _round, exploit_name = last_success_memory.get(
                            memory_key, (None, None))
                        if exploit_name:
                    ...
                    if last_successful_exploit and last_successful_exploit in current_exploits_for_service:
                        exploit_path = os.path.join(
                            exploit_directory, last_successful_exploit)
                        task_args = (exploit_path,
                                     team,
                                     rounds,
                                     round_duration,
                                     flag_format,
                        ...
                        for exploit_name in current_exploits_for_service:
                            if exploit_name != last_successful_exploit:
                                exploit_path = os.path.join(exploit_directory,
                                                            exploit_name)
                                task_args = (exploit_path,
                                             team,
                                             rounds,
                        ...
                        for exploit_name in current_exploits_for_service:
                            exploit_path = os.path.join(
                                exploit_directory, exploit_name)
                            task_args = (exploit_path,
                                         team,
                                         rounds,
                                         round_duration,
            ...
            if flags_to_send:
                ...
                try:
                    ...
                    requests.post(f"{server_url}/api/upload_flags",
                                  ...
                                  timeout=10)
                    logging.info(
                        f"{GREEN}Batch finale inviato con successo!{END}")
                except requests.exceptions.RequestException as e:
                    logging.error(
                        f"{RED}Errore nell'invio del batch finale: {e}{END}")
